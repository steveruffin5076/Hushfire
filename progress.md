...
**Immediate next step (2026-09-25, Stage 0 verification pass against `docs/DESIGN_ASSISTANT_PROMPT.md`):** PR the sector-alert horde feature first (see §8 "Local, not yet merged") — it's already owner-tested and covered by `tests/sectorAlert.test.ts`, and it's sitting uncommitted. Land it as its own small PR per the §2 rule-6 workflow before starting anything below, so it isn't carried as drift alongside new work. Everything else in this list was spot-checked against the code at `main` (PR #33) and still holds — no code exists yet for modifiers, reward choice, or Phase 7 beyond the `SessionManager.ts` stub.
