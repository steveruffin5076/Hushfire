...
// Mirrors ArmoryMenu's STEALTH_SOUND_THRESHOLD_PX ("STEALTH READY" label).
const STEALTH_READY_PX = 150;
...
describe('suppressor balance', () => {
  it('keeps the light guns stealthy', () => {
    expect(suppressedRadius('mpx')).toBeLessThanOrEqual(STEALTH_READY_PX);
    expect(suppressedRadius('glock17')).toBeLessThanOrEqual(STEALTH_READY_PX);
  });

  it('does not make the heavy guns silent', () => {
    for (const id of ['m4a1', 'shotgun', 'revolver']) {
      expect(suppressedRadius(id), id).toBeGreaterThan(STEALTH_READY_PX);
    }
  });
